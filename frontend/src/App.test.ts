...
describe('CASE UNKNOWN',()=>{
  ...
  it('opens and reopens the case briefing',async()=>{const w=mount(Investigation,{props:{game,loading:false,streamed:''}});expect(w.find('.briefing').exists()).toBe(true);expect(w.find('img[alt="블랙아웃 프로토콜 사건 현장"]').attributes('src')).toBe('/briefing/cover.jpg');expect(w.text()).toContain('장소 이동 → 단서 조사 → 용의자 고발');await w.find('.briefing-start').trigger('click');expect(w.find('.briefing').exists()).toBe(false);await w.find('.briefing-replay').trigger('click');expect(w.find('.briefing').exists()).toBe(true)})
