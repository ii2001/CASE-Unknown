...
describe('CASE UNKNOWN',()=>{
  ...
  it('plays and reopens the story briefing',async()=>{const w=mount(Investigation,{props:{game,loading:false,streamed:''}});expect(w.find('.briefing').exists()).toBe(true);expect(w.find('img[alt="블랙아웃 프로토콜 사건 현장"]').attributes('src')).toBe('/briefing/cover.jpg');await w.find('.briefing-skip').trigger('click');expect(w.text()).toContain('수사 지침');await w.find('.briefing-start').trigger('click');expect(w.find('.briefing').exists()).toBe(false);await w.find('.briefing-replay').trigger('click');expect(w.find('.briefing').exists()).toBe(true)})
