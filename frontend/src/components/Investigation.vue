...
const input=ref(''); const tab=ref<'evidence'|'suspects'>('evidence'); const showBriefing=ref(!props.game.completed); const showAccuse=ref(false); const selected=ref(''); const reasoning=ref('')
const location=computed(()=>props.game.locations.find(x=>x.id===props.game.current_location_id)!)
function openBriefing(){showBriefing.value=true}
...
<template><div class="game-shell">
  ...
  <div v-if="showBriefing" class="briefing" role="dialog" aria-modal="true" aria-labelledby="briefing-title">
    <img class="briefing-backdrop" :src="'/briefing/cover.jpg'" :alt="`${game.title} 사건 현장`"><div class="briefing-shade"></div>
    <section class="briefing-summary">
      <small>CASE {{game.id.slice(0,8).toUpperCase()}} · INCIDENT BRIEFING</small>
      <h1 id="briefing-title">{{game.title}}</h1><p>{{game.incident_summary}}</p>
      <div class="briefing-actions"><span>장소 이동 → 단서 조사 → 용의자 고발</span><button class="briefing-start" @click="showBriefing=false">수사 시작 →</button></div>
    </section>
  </div>
